...
def der_decode_sig(sig):
    ...
    rightlen = decode(sig[sig_s_len_start:sig_s_len_end], 16) * 2
...
def deserialize_script(script):
    ...
    while pos < len(script):
        ...
        if code == 0:
            ...
            data_start = pos + 1 + szsz
            data_end = data_start + sz
            out.append(script[data_start:data_end])
            pos += 1 + szsz + sz
        elif code <= 96:
            out.append(code - 80)
            pos += 1
...
def verify_tx_input(tx, i, script, sig, pub, hashcode=SIGHASH_ALL):
    ...
    if isinstance(script, str) and re.match("^[0-9a-fA-F]*$", script):
        script = safe_unhexlify(script)
    if not isinstance(sig, str) and re.match("^[0-9a-fA-F]*$", sig):
...
def mktx(tm=time.time(), *args):  # [in0, in1.],[out0, out1.] or in0, in1 . out0 out1 .
    ...
    txobj = {"version": 1, "time": int(tm), "ins": [], "outs": [], "locktime": 0}
    for i in ins:
    ...
    for o in outs:
        if isinstance(o, str):
            ...
            o = {"address": addr, "value": int(o[val_start:])}
