...
'(For msg message via API) Doing proof of work. Total required difficulty:', float(requiredAverageProofOfWorkNonceTrialsPerByte) / defaults.networkDefaultProofOfWorkNonceTrialsPerByte, 'Required small message difficulty:', float(requiredPayloadLengthExtraBytes) / defaults.networkDefaultPayloadLengthExtraBytes'(For msg message via API) Doing proof of work. Total required difficulty:', float(requiredAverageProofOfWorkNonceTrialsPerByte) / defaults.networkDefaultProofOfWorkNonceTrialsPerByte, 'Required small message difficulty:', float(requiredPayloadLengthExtraBytes) / defaults.networkDefaultPayloadLengthExtraBytes
        powStartTime = time.time()
        initialHash = hashlib.sha512(encryptedPayload).digest()
        trialValue, nonce = proofofwork.run(target, initialHash)
        with shared.printLock:
'(For msg message via API) Found proof of work', trialValue, 'Nonce:', nonce'(For msg message via API) Found proof of work', trialValue, 'Nonce:', nonce
            try:
'POW took', int(time.time() - powStartTime), 'seconds.', nonce / (time.time() - powStartTime), 'nonce trials per second.''POW took', int(time.time() - powStartTime), 'seconds.', nonce / (time.time() - powStartTime), 'nonce trials per second.'
            except:
                pass
        ...
        with shared.printLock:
'Broadcasting inv for msg(API disseminatePreEncryptedMsg command):', hexlify(inventoryHash)'Broadcasting inv for msg(API disseminatePreEncryptedMsg command):', hexlify(inventoryHash)
...
'(For pubkey message via API) Doing proof of work.''(For pubkey message via API) Doing proof of work.'
        initialHash = hashlib.sha512(payload).digest()
        trialValue, nonce = proofofwork.run(target, initialHash)
'(For pubkey message via API) Found proof of work', trialValue, 'Nonce:', nonce'(For pubkey message via API) Found proof of work', trialValue, 'Nonce:', nonce
        ...
        with shared.printLock:
'broadcasting inv within API command disseminatePubkey with hash:', hexlify(inventoryHash)'broadcasting inv within API command disseminatePubkey with hash:', hexlify(inventoryHash)
