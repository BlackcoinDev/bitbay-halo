...
def blockr_unspent(*args):
    ...
    for dat in data:
        for u in dat["unspent"]:
            o.append({"output": u["tx"] + ":" + str(u["n"]), "value": int(u["amount"].replace(".", ""))})
    return o
...
unspent_getters = {"bci": bci_unspent, "blockr": blockr_unspent, "helloblock": helloblock_unspent}
...
pushtx_getters = {"bci": bci_pushtx, "blockr": blockr_pushtx, "helloblock": helloblock_pushtx}
...
def bci_fetchtx(txhash):
    ...
    if not re.match("^[0-9a-fA-F]*$", txhash):
        txhash = txhash.hex() if isinstance(txhash, bytes) else txhash
...
fetchtx_getters = {"bci": bci_fetchtx, "blockr": blockr_fetchtx, "helloblock": helloblock_fetchtx}
...
block_header_data_getters = {"bci": bci_get_block_header_data, "blockr": blockr_get_block_header_data}
