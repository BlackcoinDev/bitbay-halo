                    ...
                    if r == d.DIALOG_OK:
                        if t == "1":  # Create new address
                            ...
                            r, t = d.menu(
                                ...
                                choices=[("1", "Use a random number generator"), ("2", "Use a passphrase")],
                            )
                            if r == d.DIALOG_OK:
                                if t == "1":
                            ...
                            if shared.safeConfigGetBoolean(a, "chan"):
                                d.scrollbox(
                                    str("This is a chan address. You cannot use it as a pseudo-mailing list."),
                                    exit_label="Continue",
                                )
                            else:
                        ...
                        if t == "1":
                            ...
                            if r == d.DIALOG_OK:
                                ...
                                if addr not in [item[1] for i, item in enumerate(addrbook)]:
                                    ...
                                    d.scrollbox(
                                        str("The selected address is already in the Address Book."),
                                        exit_label="Continue",
                                    )
                        elif t == "4":
...
def sendMessage(sender="", recv="", broadcast=None, subject="", body="", reply=False):
    ...
    if not broadcast:
            ...
            if addr != "":
                ...
                if status != "success":
                    ...
                    if version > 4 or version <= 1:
                        d.set_background_title("Recipient address error")
                        d.scrollbox(
                            str("Could not understand version number " + version + "of address" + addr + "."),
                            exit_label="Continue",
                        )
                        continue
        ...
        if recv == "":
            ...
            d.scrollbox(str("You must specify an address to send the message from."), exit_label="Continue")
        else:
            ackdata = OpenSSL.rand(32)
            recv = BROADCAST_STR
            ripe = ""
