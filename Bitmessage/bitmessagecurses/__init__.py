                ...
                if menutab == 1:
                    ...
                    if r == d.DIALOG_OK:
                        if t == "1": # View
                            ...
                            if ret != []:
                                ...
                                d.scrollbox(str(ascii(msg)), 30, 80, exit_label="Continue")
                                sqlExecute("UPDATE inbox SET read=1 WHERE msgid=?", inbox[inboxcur][0])
                                inbox[inboxcur][7] = 1
                            else:
                                d.scrollbox(str("Could not fetch message."), exit_label="Continue")
                            ...
                            if not addresses[i][1]:
                                d.scrollbox(str("Sending address disabled, please either enable it or choose a different address."), exit_label="Continue")
                                return
                            toaddr = m[2]
                            if ischan:
                                toaddr = fromaddr
                            ...
                            del inbox[inboxcur]
                            d.scrollbox(str("Message moved to trash. There is no interface to view your trash, \nbut the message is still on disk if you are desperate to recover it."),
                                exit_label="Continue")
                elif menutab == 2:
                    a = ""
                    ...
                    if r == d.DIALOG_OK:
                        if t == "1": # Create new address
                            ...
                            d.scrollbox(str("Here you may generate as many addresses as you like.\n"
                                    ...
                                    if r == d.DIALOG_OK:
                                        if t[0] == t[1]:
                                            ...
                                            if r == d.DIALOG_OK:
                                                ...
                                                if r == d.DIALOG_OK and "1" in t:
                                                    shorten = True
                                                d.scrollbox(str("In addition to your passphrase, be sure to remember the following numbers:\n"
                                            ...
                                            d.scrollbox(str("Passphrases do not match"), exit_label="Continue")
                            ...
                            if shared.safeConfigGetBoolean(a, "chan"):
                                d.scrollbox(str("This is a chan address. You cannot use it as a pseudo-mailing list."), exit_label="Continue")
                            else:
...
def sendMessage(sender="", recv="", broadcast=None, subject="", body="", reply=False):
    ...
    if not broadcast:
        ...
        for addr in recvlist:
            if addr != "":
                ...
                if status != "success":
                    ...
                    d.scrollbox(str(err), exit_label="Continue")
                else:
                    addr = addBMIfNotPresent(addr)
                    if version > 4 or version <= 1:
                        d.set_background_title("Recipient address error")
                        d.scrollbox(str("Could not understand version number " + version + "of address" + addr + "."),
                            exit_label="Continue")
                        continue
                    if stream > 1 or stream == 0:
                        d.set_background_title("Recipient address error")
                        d.scrollbox(str("Bitmessage currently only supports stream numbers of 1, unlike as requested for address " + addr + "."),
                            exit_label="Continue")
                        continue
                    if len(shared.connectedHostsList) == 0:
                        d.set_background_title("Not connected warning")
                        d.scrollbox(str("Because you are not currently connected to the network, "),
                            exit_label="Continue")
        ...
        if recv == "":
            ...
            d.scrollbox(str("You must specify an address to send the message from."),
                exit_label="Continue")
        else:
            ackdata = OpenSSL.rand(32)
            recv = BROADCAST_STR
