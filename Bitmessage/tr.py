    ...
    def arg(self, argument):
        if "%" in self.text:
            return translateClass(
                self.context, self.text.replace("%", "", 1)
            )  # This doesn't actually do anything with the arguments because we don't have a UI in which to display this information anyway.
        else:
            return self.text
