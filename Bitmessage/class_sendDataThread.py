...
class sendDataThread(threading.Thread):
    ...
    def setup(self, sock, HOST, PORT, streamNumber, someObjectsOfWhichThisRemoteNodeIsAlreadyAware):
        self.sock = sock
        ...
        try:
            ...
            # We used to send our addr packet here (and still do for protocol <=2)
            # but now we wait for the verack.
        except Exception as err:
            # print 'socket error in sendDataThread setup'
            import traceback
    ...
    def run(self):
        with shared.printLock:
            print(
                (
                    "sendDataThread starting. ID:",
                    str(id(self)) + ". Number of queues in sendDataQueues:",
            ...
            if deststream == self.streamNumber or deststream == 0:
                if command == "shutdown":
                    ...
                    if shared.verbose >= 2:
                        print(
                            (
                                "setting the stream number in the sendData thread (ID:",
                                id(self),
                                ") to",
                                self.streamNumber,
                            )
                    ...
                    if shared.verbose >= 2:
                        print(
                            (
                                ...
                                id(self),
                                ") to",
                                specifiedRemoteProtocolVersion,
                            )
                        )
