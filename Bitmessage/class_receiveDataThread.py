...
class receiveDataThread(threading.Thread):
    ...
    def setup(
        ...
        shared.connectedHostsList[self.peer.host] = (
            0  # The very fact that this receiveData thread exists shows that we are connected to the remote host. Let's add it to this list so that an outgoingSynSender thread doesn't try to connect to it.
        )
    ...
    def run(self):
        with shared.printLock:
            print(
                (
                    "receiveDataThread starting. ID",
                    str(id(self)) + ". The size of the shared.connectedHostsList is now",
        ...
        while True:
            ...
            try:
                ...
                with shared.printLock:
                    print(
                        (
                            "Timeout occurred waiting for data from",
                            self.peer,
                ...
                with shared.printLock:
                    print(
                        (
                            "sock.recv error. Closing receiveData thread (" + str(self.peer) + ", Thread ID:",
                            str(id(self)) + ").",
                            err,
                        )
                    )
    ...
    def processData(self):
        if (
            len(self.data) < shared.Header.size
        ):  # if so little of the data has arrived that we can't even read the checksum then wait for more data.
            return
        ...
        if payloadLength > 1600100:  # ~1.6 MB which is the maximum possible size of an inv message.
            ...
            del (
                ...
                payloadLength,
                checksum,
            )  # we don't need these anymore and better to clean them now before the recursive call rather than after
            self.processData()
            return
        ...
        if (
            self.initiatedConnection and self.connectionIsOrWasFullyEstablished
        ):  # The remote port is only something we should share with others if it is the remote node's incoming port (rather than some random operating-system-assigned outgoing port).
            ...
            if command == "error":
                ...
                if command == "addr":
                    ...
                    if shared.verbose >= 2:
                        with shared.printLock:
                            print("Sending pong")
                    self.sendDataThreadQueue.put((0, "pong", ""))
                elif command == "pong":
                    # The packet is empty
                    pass
                elif command == "verack":
                    ...
                    self.sendDataThreadQueue.put(
                        (0, "sendaddr", shared.recentNodeList())
                    )  # This is a request to the sendDataThread to calculate the addr message and put it into its own queue.
        except varintDecodeError as e:
            logger.debug(
                "There was a problem with a varint while processing a message from the wire. Some details: %s" % e
            )
        ...
        del payload
        self.data = self.data[
            payloadLength + shared.Header.size :
        ]  # take this message out and then process the next message

        if self.data == "":  # if there are no more messages
            while len(self.objectsThatWeHaveYetToGetFromThisPeer) > 0:
                shared.numberOfInventoryLookupsPerformed += 1
                (objectHash,) = random.sample(self.objectsThatWeHaveYetToGetFromThisPeer, 1)
                if objectHash in shared.inventory:
                    ...
                    del self.objectsThatWeHaveYetToGetFromThisPeer[
                        objectHash
                    ]  # It is possible that the remote node might not respond with the object. In that case, we'll very likely get it from someone else anyway.
                        ...
                        with shared.printLock:
                            print(
                                (
                                    "(concerning",
                                    str(self.peer) + ")",
                                    "number of objectsThatWeHaveYetToGetFromThisPeer is now 0",
                                )
                            )
                        try:
                ...
                with shared.printLock:
                    print(
                        (
                            "(concerning",
                            str(self.peer) + ")",
                            "number of objectsThatWeHaveYetToGetFromThisPeer is now",
                            len(self.objectsThatWeHaveYetToGetFromThisPeer),
                        )
                    )
    ...
    def connectionFullyEstablished(self):
        ...
        self.sock.settimeout(
            600
        )  # We'll send out a pong every 5 minutes to make sure the connection stays alive if there has been no other traffic to send lately.
        shared.UISignalQueue.put(("updateNetworkStatusTab", "no data"))
        with shared.printLock:
        ...
        shared.broadcastToSendDataQueues((self.streamNumber, "advertisepeer", dataToSend))
    ...
    def sendBigInv(self):
        ...
        queryreturn = sqlQuery(
            """SELECT hash FROM inventory WHERE expirestime>? and streamnumber=?""", int(time.time()), self.streamNumber
        )
        bigInvList = {}
        ...
        for hash, storedValue in list(bigInvList.items()):
            payload += hash
            numberOfObjectsInInvMessage += 1
            if (
                numberOfObjectsInInvMessage == 50000
            ):  # We can only send a max of 50000 items per inv message but we may have more objects to advertise. They must be split up into multiple inv messages.
    ...
    def recerror(self, data):
        ...
        if inventoryVector:
            message += " This concerns object %s" % inventoryVector.hex()
        if banTime > 0:
    ...
    def recinv(self, data):
        totalNumberOfobjectsThatWeHaveYetToGetFromAllPeers = (
            0  # this counts duplicates separately because they take up memory
        )
        if len(shared.numberOfObjectsThatWeHaveYetToGetPerPeer) > 0:
            for key, value in list(shared.numberOfObjectsThatWeHaveYetToGetPerPeer.items()):
                totalNumberOfobjectsThatWeHaveYetToGetFromAllPeers += value
            with shared.printLock:
                print(
                    (
                        "number of keys(hosts) in shared.numberOfObjectsThatWeHaveYetToGetPerPeer:",
                        len(shared.numberOfObjectsThatWeHaveYetToGetPerPeer),
                    )
                )
                print(
                    (
        ...
        if numberOfItemsInInv == 1:  # we'll just request this data from the person who advertised the object.
            if (
                totalNumberOfobjectsThatWeHaveYetToGetFromAllPeers > 200000
                and len(self.objectsThatWeHaveYetToGetFromThisPeer) > 1000
                ...
                return
            self.someObjectsOfWhichThisRemoteNodeIsAlreadyAware[data[lengthOfVarint : 32 + lengthOfVarint]] = 0
            shared.numberOfInventoryLookupsPerformed += 1
            ...
            logger.info(
                "inv message lists %s objects. Of those %s are new to me. It took %s seconds to figure that out.",
                numberOfItemsInInv,
                len(objectsNewToMe),
                time.time() - startTime,
            )
                    ...
                    with shared.printLock:
                        print(
                            (
                                "We already have",
                                totalNumberOfobjectsThatWeHaveYetToGetFromAllPeers,
                                "items yet to retrieve from peers and over",
                                len(self.objectsThatWeHaveYetToGetFromThisPeer),
                                "from this node in particular. Ignoring the rest of this inv message.",
                            )
                        )
                    break
                self.someObjectsOfWhichThisRemoteNodeIsAlreadyAware[item] = (
                    0  # helps us keep from sending inv messages to peers that already know about the objects listed therein
                )
                self.objectsThatWeHaveYetToGetFromThisPeer[item] = (
    ...
    def sendgetdata(self, hash):
        with shared.printLock:
            print(("sending getdata to retrieve object with hash:", hash.hex()))
        payload = b"\x01" + hash
    ...
    def recgetdata(self, data):
        ...
        if len(data) < lengthOfVarint + (32 * numberOfRequestedInventoryItems):
            ...
            return
        for i in range(numberOfRequestedInventoryItems):
            hash = data[lengthOfVarint + (i * 32) : 32 + lengthOfVarint + (i * 32)]
            with shared.printLock:
                print(("received getdata request for item:", hash.hex()))
            ...
            if hash in shared.inventory:
                ...
                shared.inventoryLock.release()
                queryreturn = sqlQuery(
                    """select payload from inventory where hash=? and expirestime>=?""", hash, int(time.time())
                )
                if queryreturn != []:
                    for row in queryreturn:
                        (payload,) = row
    ...
    def recaddr(self, data):
        ...
        for i in range(0, numberOfAddressesIncluded):
            try:
                ...
                with shared.printLock:
                    sys.stderr.write("ERROR TRYING TO UNPACK recaddr (recaddrHost). Message: %s\n" % str(err))
                break  # giving up on unpacking any more. We should still be connected however.

            try:
                (recaddrStream,) = unpack(
                    ">I", data[8 + lengthOfNumberOfAddresses + (38 * i) : 12 + lengthOfNumberOfAddresses + (38 * i)]
                )
            except Exception as err:
                with shared.printLock:
            ...
            if recaddrStream == 0:
                continue
            if (
                recaddrStream != self.streamNumber
                and recaddrStream != (self.streamNumber * 2)
                and recaddrStream != ((self.streamNumber * 2) + 1)
            ...
            try:
                (recaddrServices,) = unpack(
                    ">Q", data[12 + lengthOfNumberOfAddresses + (38 * i) : 20 + lengthOfNumberOfAddresses + (38 * i)]
                )
            except Exception as err:
                with shared.printLock:
            ...
            try:
                (recaddrPort,) = unpack(
                    ">H", data[36 + lengthOfNumberOfAddresses + (38 * i) : 38 + lengthOfNumberOfAddresses + (38 * i)]
                )
            except Exception as err:
                with shared.printLock:
            ...
            # recaddrPort, ', i', i
            if fullHost[0:12] == b"\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\xff\xff":
            ...
            (timeSomeoneElseReceivedMessageFromThisNode,) = unpack(
                ">Q", data[lengthOfNumberOfAddresses + (38 * i) : 8 + lengthOfNumberOfAddresses + (38 * i)]
            )  # This is the 'time' value in the received addr message. 64-bit.
            if (
                recaddrStream not in shared.knownNodes
            ):  # knownNodes is a dictionary of dictionaries with one outer dictionary for each stream. If the outer stream dictionary doesn't exist yet then we must make it.
                shared.knownNodesLock.acquire()
            ...
            if peerFromAddrMessage not in shared.knownNodes[recaddrStream]:
                if (
                    len(shared.knownNodes[recaddrStream]) < 20000
                    and timeSomeoneElseReceivedMessageFromThisNode > (int(time.time()) - 10800)
                    and timeSomeoneElseReceivedMessageFromThisNode < (int(time.time()) + 10800)
                    ...
                    shared.broadcastToSendDataQueues((self.streamNumber, "advertisepeer", hostDetails))
            else:
                timeLastReceivedMessageFromThisNode = shared.knownNodes[recaddrStream][
                    peerFromAddrMessage
                ]  # PORT in this case is either the port we used to connect to the remote node, or the port that was specified by someone else in a past addr message.
                if (timeLastReceivedMessageFromThisNode < timeSomeoneElseReceivedMessageFromThisNode) and (
                    timeSomeoneElseReceivedMessageFromThisNode < int(time.time())
                ):
                    shared.knownNodesLock.acquire()
                    shared.knownNodes[recaddrStream][peerFromAddrMessage] = timeSomeoneElseReceivedMessageFromThisNode
                    shared.knownNodesLock.release()
    ...
    def sendaddr(self):
        ...
        for (HOST, PORT), value in list(addrsInMyStream.items()):
        ...
        for (HOST, PORT), value in list(addrsInChildStreamLeft.items()):
            timeLastReceivedMessageFromThisNode = value
            if timeLastReceivedMessageFromThisNode > (
                int(time.time()) - shared.maximumAgeOfNodesThatIAdvertiseToOthers
            ):  # If it is younger than 3 hours old.
                ...
                payload += shared.encodeHost(HOST)
                payload += pack(">H", PORT)  # remote port
        for (HOST, PORT), value in list(addrsInChildStreamRight.items()):
            timeLastReceivedMessageFromThisNode = value
            if timeLastReceivedMessageFromThisNode > (
                int(time.time()) - shared.maximumAgeOfNodesThatIAdvertiseToOthers
            ):  # If it is younger than 3 hours old.
    ...
    def recversion(self, data):
        ...
        if timeOffset > 3600:
            ...
            logger.info(
                "%s's time is too far in the future (%s seconds). Closing connection to it." % (self.peer, timeOffset)
            )
            time.sleep(2)
            self.sendDataThreadQueue.put((0, "shutdown", "no data"))
            return
        ...
        if self.streamNumber != 1:
            ...
            with shared.printLock:
                print(
                    ("Closed connection to", self.peer, "because they are interested in stream", self.streamNumber, ".")
                )
            return
        shared.connectedHostsList[self.peer.host] = (
            1  # We use this data structure to not only keep track of what hosts we are connected to so that we don't try to connect to them again, but also to list the connections count on the Network Status tab.
        )
        ...
        shared.knownNodesLock.acquire()
        shared.knownNodes[self.streamNumber][shared.Peer(self.peer.host, self.remoteNodeIncomingPort)] = int(
            time.time()
        )
        shared.needToWriteKnownNodesToDisk = True
        shared.knownNodesLock.release()
