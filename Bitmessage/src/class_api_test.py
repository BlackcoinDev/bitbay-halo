...
class API_TEST(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        global api
        api = class_api.getAPI(workingdir=path)
        time.sleep(5)
    ...
    def test_06_send_messages(self):
        ...
        assert api.getSentMessageByAckData(ackdata1)[0]["status"] in [
            "msgqueued",
            "broadcastqueued",
            "broadcastsent",
            "doingpubkeypow",
            ...
            "ackreceived",
        ], api.getSentMessageByAckData(ackdata1)
        assert api.getSentMessageByAckData(ackdata2)[0]["status"] in [
            "msgqueued",
            "broadcastqueued",
            "broadcastsent",
            "doingpubkeypow",
            ...
            "msgsentnoackexpected",
            "ackreceived",
        ], api.getSentMessageByAckData(ackdata2)
