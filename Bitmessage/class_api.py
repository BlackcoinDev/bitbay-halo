...
###############
from . import addresses, helper_inbox, helper_sent, parallelTestModule
...
def getAPI(workingdir=None, silent=False):
    ...
    class MainAPI(bitmessagemain.Main):
        ...
        def createDeterministicAddresses(
            self,
            passphrase,
            label="",
            numberOfAddresses=1,
            ...
            if numberOfAddresses > 999:
                raise APIError(
                    "You have (accidentally?) specified too many addresses to make. Maximum 999. \
        ...
        def listAddresses(self):
            ...
            for addressInKeysFile in configSections:
                if addressInKeysFile != "bitmessagesettings":
                    ...
                    address1.append(
                        {
                            "label": bitmessagemain.shared.config.get(addressInKeysFile, "label"),
                            "address": addressInKeysFile,
                            "stream": streamNumber,
                            "enabled": bitmessagemain.shared.config.getboolean(addressInKeysFile, "enabled"),
                        }
                    )
            return address1
