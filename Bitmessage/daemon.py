...
def configInit():
    ...
    )  # Sets the bitmessage port to stop the warning about the api not properly being setup. This is in the event that the keys.dat is in a different directory or is created locally to connect to a machine remotely.
    config.set("bitmessagesettings", "apienabled", "true")  # Sets apienabled to true in keys.dat

    with open(keysName, "w") as configfile:
...
def apiInit(apiEnabled):
    ...
    if apiEnabled == False:  # API information there but the api is disabled.
        ...
        if uInput == "y":  #
            config.set("bitmessagesettings", "apienabled", "true")  # Sets apienabled to true in keys.dat
            with open(keysPath, "w") as configfile:
                config.write(configfile)
...
def listAdd():  # Lists all of the addresses and their info
    ...
    for addNum in range(0, numAddresses):  # processes all of the addresses and lists them out
        ...
        print(
            "     |" + str(addNum).ljust(3) + "|" + label.ljust(19) + "|" + address.ljust(37) + "|" + stream.ljust(1),
            "|" + enabled.ljust(7) + "|",
        )
...
def attachment():  # Allows users to attach a file to their message or broadcast
    ...
    while True:
        ...
        filetype = imghdr_what(filePath)  # Tests if it is an image file
...
def inbox(
    ...
    for msgNum in range(0, numMessages):  # processes all of the messages in the inbox
        ...
        if not unreadOnly or not message["read"]:
            ...
            print(
                "     Received:",
                datetime.datetime.fromtimestamp(float(message["receivedTime"])).strftime("%Y-%m-%d %H:%M:%S"),
            )
            messagesPrinted += 1
            if not message["read"]:
                messagesUnread += 1
...
def replyMsg(msgNum, forwardORreply):  # Allows you to reply to the message you are currently on. Saves typing in the addresses and subject.
    ...
    newMessage = base64.b64encode(newMessage.encode("utf-8")).decode("ascii")
        ...
        for entry in addressBook["addresses"]:
            if entry["address"] not in knownAddresses:
                knownAddresses[entry["address"]] = "%s (%s)" % (
                    base64.b64decode(entry["label"]).decode("utf-8", "ignore"),
                    entry["address"],
                )
    except:
        print("\n     Connection Error\n")
...
def listAddressBookEntries():
    try:
        ...
        print("     |--------------------|---------------------------------------|")
        for entry in addressBook["addresses"]:
            label = base64.b64decode(entry["label"]).decode("utf-8", "ignore")
